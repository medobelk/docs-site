...
<?}?>
				...
				<a class="more" href="<?=Route::buildURL('QA', 'getlist');?>">Читать дальше</a>
				...
				<form method="POST" action="<?=Route::buildURL('QA', 'request');?>">
					<label>Имя</label><input type="text" name="name" value="<?=$this->name;?>" /><br />
					<label>Электронный адрес</label><input type="text" name="email" value="<?=$this->email;?>" /><br />
					<label>Суть проблемы</label><textarea name="text"><?=$this->text;?></textarea><br />
