...
@section('content')
                                        ...
                                        <td class="no-sort no-click" id="bread-actions">
                                            ...
                                            @can('edit', $data)
                                                @if( $data->status === 'PENDING' )
                                                    <a data-toggle="popover" data-placement="top" data-id="{{ $data->{$data->getKeyName()} }}" class="btn btn-sm btn-success pull-right edit accept-review" id="accept-{{ $data->{$data->getKeyName()} }}">
                                                        <i class="voyager-check"></i> <span class="hidden-xs hidden-sm">Опубликовать</span>
                                                    </a>
                                                @else
                                                    <a data-toggle="popover" data-placement="top" data-id="{{ $data->{$data->getKeyName()} }}" class="btn btn-sm btn-warning pull-right edit hide-review" id="hide-{{ $data->{$data->getKeyName()} }}">
                                                        <i class="voyager-bookmark"></i> <span class="hidden-xs hidden-sm">Скрыть</span>
                                                    </a>
                                                @endif
                                            @endcan
                                            @can('read', $data)
    ...
    <div class="modal modal-success fade" tabindex="-1" id="accept_modal" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('voyager.generic.close') }}"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-check"></i> Вы уверены что хотите опубликовать это? </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ url('admin-cus/review-accept') }}" id="form-accept-review" method="POST">
                        {{ csrf_field() }}
    ...
    <div class="modal modal-warning fade" tabindex="-1" id="hide_modal" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('voyager.generic.close') }}"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-bookmark"></i> Вы уверены что хотите скрыть это? </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ url('admin-cus/review-hide') }}" id="form-hide-review" method="POST">
                        {{ csrf_field() }}
...
@section('javascript')
    ...
    <script>
        ...
        var acceptFormAction;
        $('td').on('click', '.accept-review', function (e) {
            var form = $('#form-accept-review')[0];

            if (!acceptFormAction) { // Save form action initial value
                acceptFormAction = form.action;
            }

            form.action = acceptFormAction + '/' + $(this).data('id');

            $('#accept_modal').modal('show');
        });

        var hideFormAction;
        $('td').on('click', '.hide-review', function (e) {
            var form = $('#form-hide-review')[0];

            if (!hideFormAction) { // Save form action initial value
                hideFormAction = form.action;
            }

            form.action = hideFormAction + '/' + $(this).data('id');

            $('#hide_modal').modal('show');
        });

        $("th:contains('Status')").text('Статус');
        $("th:contains('Body')").text('Содержание');
        $("th:contains('Authority')").text('Автор');
        $("th:contains('Created At')").text('Дата Создания');
    </script>
@stop
