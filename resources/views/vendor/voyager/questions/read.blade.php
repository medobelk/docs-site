                ...
                <div class="panel panel-bordered" style="padding-bottom:5px;">
                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            <form class="form-edit-add" action="{{ url('/create-answer') }}" method="post">
                                {{ csrf_field() }}
                                <textarea class="form-control" data-name="Complaints" name="complaints"></textarea>
                                <input type="hidden" name="question_id" value="{{ $dataTypeContent->getKey() }}">
                            </form>
                            <button class="btn btn-success btn-add-new save" onclick="submitForm()">Отправить</button>
                        </div>    
                    </div>
                    <div class="panel-heading" style="border-bottom: none;">
                        <h3 class="panel-title">Ответы</h3>
                    </div>
                    @foreach( $answers as $key => $answer )
                        <div class="panel-body">
                            <p>Author: {{ $answer['user']['name'] }} at @php $answerDate = new DateTime($answer['created_at']); echo $answerDate->format('l F Y') . ' on ' . $answerDate->format('H:i'); @endphp</p>
                            <span>{{ $answer['body'] }}</span>
                            <p><a class="btn btn-primary save" href="{{ url('/admin/answers/'.$answer['id'].'/edit') }}" ">Редактировать</a></p>
                        </div>
                    @endforeach    
                </div>
            </div>
